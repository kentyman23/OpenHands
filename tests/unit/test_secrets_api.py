    ...
    with patch(
        ...
        add_providers_data = {
            'provider_tokens': {
                ...
                'azure_devops': {'token': 'azure-devops-token', 'host': 'dev.azure.enterprise.com'},
            }
        }
        ...
        assert (
            stored_secrets.provider_tokens[ProviderType.AZURE_DEVOPS].token.get_secret_value()
            == 'azure-devops-token'
        )
        assert (
            stored_secrets.provider_tokens[ProviderType.AZURE_DEVOPS].host
            == 'dev.azure.enterprise.com'
        )
