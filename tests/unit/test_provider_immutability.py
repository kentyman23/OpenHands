...
async def test_get_env_vars():
    ...
    assert env_vars[ProviderType.AZURE_DEVOPS].get_secret_value() == 'azure_devops_token'
    ...
    assert ProviderType.AZURE_DEVOPS not in env_vars
    ...
    assert exposed_vars['azure_devops_token'] == 'azure_devops_token'
...
def test_check_cmd_action_for_provider_token_ref():
    ...
    cmd = CmdRunAction(command='echo $GITHUB_TOKEN && echo $GITLAB_TOKEN && echo $AZURE_DEVOPS_TOKEN')
    providers = ProviderHandler.check_cmd_action_for_provider_token_ref(cmd)
    assert ProviderType.GITHUB in providers
    assert ProviderType.GITLAB in providers
    assert ProviderType.AZURE_DEVOPS in providers
...
def test_get_provider_env_key():
    ...
    assert ProviderHandler.get_provider_env_key(ProviderType.AZURE_DEVOPS) == 'azure_devops_token'
