...
def update_existing_pull_request(
    ...
    if platform == ProviderType.GITHUB:
        ...
        handler = ServiceContextIssue(
            AzureDevOpsIssueHandler(issue.owner, issue.repo, token, username, base_domain),
            llm_config,
        )
    else:
        raise ValueError(f'Unsupported platform: {platform}')
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description='Send a pull request to Github, Gitlab, Bitbucket, or Azure DevOps.'
    )
    parser.add_argument(
        '--selected-repo',
        type=str,
    ...
    parser.add_argument(
        '--reviewer',
        type=str,
        help='GitHub, GitLab, Bitbucket, or Azure DevOps username of the person to request review from',
        default=None,
    )
    parser.add_argument(
        '--pr-title',
    ...
    token = my_args.token or os.getenv('GITHUB_TOKEN') or os.getenv('GITLAB_TOKEN') or os.getenv('BITBUCKET_TOKEN') or os.getenv('AZURE_DEVOPS_TOKEN')
    if not token:
        raise ValueError(
            'token is not set, set via --token or GITHUB_TOKEN, GITLAB_TOKEN, BITBUCKET_TOKEN, or AZURE_DEVOPS_TOKEN environment variable.'
        )
